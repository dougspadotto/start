resource "databricks_group" "mlops-service-principal-group-staging" {
  display_name = "yape-mlstacks-2-service-principals"
...
module "azure_create_sp" {
  ...
  service_principal_name       = "yape-mlstacks-2-cicd"
  project_directory_path       = "/yape-mlstacks-2"
  azure_tenant_id              = var.azure_tenant_id
  service_principal_group_name = "yape-mlstacks-2-service-principals"
}
...
provider "databricks" {
  alias = "staging_sp"
  host  = "https://adb-6832525285606128.8.azuredatabricks.net"
  token = module.azure_create_sp.staging_service_principal_aad_token
}

provider "databricks" {
  alias = "prod_sp"
  host  = "https://adb-2529239831454482.2.azuredatabricks.net"
