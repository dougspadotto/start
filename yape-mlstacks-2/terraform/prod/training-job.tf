resource "databricks_job" "model_training_job" {
  name = "${local.env_prefix}yape-mlstacks-2-model-training-job"
  ...
  task {
    ...
    notebook_task {
      notebook_path = "yape-mlstacks-2/training/notebooks/Train"
      base_parameters = {
        env = local.env
      }
    }
  ...
  task {
    ...
    notebook_task {
      notebook_path = "yape-mlstacks-2/validation/notebooks/ModelValidation"
      base_parameters = {
        env             = local.env
        ...
        # Specifies the name of the function in yape-mlstacks-2/validation/validation.py that returns custom metrics.
        # TODO(optional): custom_metrics_loader_function
        custom_metrics_loader_function = "custom_metrics"
        # Specifies the name of the function in yape-mlstacks-2/validation/validation.py that returns model validation thresholds.
        # TODO(optional): validation_thresholds_loader_function
        validation_thresholds_loader_function = "validation_thresholds"
        # Specifies the name of the function in yape-mlstacks-2/validation/validation.py that returns evaluator_config.
  ...
  task {
    ...
    notebook_task {
      notebook_path = "yape-mlstacks-2/deployment/model_deployment/notebooks/TriggerModelDeploy"
      base_parameters = {
        env = local.env
      }
    }
