...
This directory (`yape-mlstacks-2/terraform`) includes Databricks ML resource configuration, i.e. the definitions of a batch
...
the contents of `yape-mlstacks-2/terraform` to the Git repo:
```
...
Then, resolve the TODOs in `yape-mlstacks-2/terraform/staging/inference-job.tf` and `yape-mlstacks-2/terraform/prod/inference-job.tf`, passing
...
To get started, open `yape-mlstacks-2/terraform/staging/inference-job.tf`.  The file contains the ML resource definition of
a batch inference job, like:

```$xslt
resource "databricks_job" "batch_inference_job" {
  name = "${local.env_prefix}yape-mlstacks-2-batch-inference-job"
  ...
  notebook_task {
    notebook_path = "yape-mlstacks-2/deployment/batch_inference/notebooks/BatchInference"
  }
  ...
}
```

The example above defines a Databricks job with name `staging-yape-mlstacks-2-batch-inference-job`
that runs the notebook under `yape-mlstacks-2/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model
for batch inference. 
